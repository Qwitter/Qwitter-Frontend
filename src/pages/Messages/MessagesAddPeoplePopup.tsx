                ...
                <div className="w-full h-full items-start flex flex-col max-h-[calc(100vh-55px)]">
                    ...
                    <div className="overflow-y-auto w-full ">
                        {peopleSearchText.length > 0 ? <ShowUsersSuggestions onUserClick={handlePickUser} selectedUsers={selectedUsers} users={data!} /> :
                            ...
                            <ShowAllUserConversations onUserClick={handlePickUser} selectedUsers={selectedUsers} />
                        }
                    </div>
                </div>
            }
...
function ShowAllUserConversations({ onUserClick, selectedUsers }: { selectedUsers: MessageUser[], onUserClick: (user: MessageUser) => void }) {
    ...
    return (
        <ul className="flex-shrink min-h-[60vh]" >
            ...
            {
                userAllConversation && userAllConversation.map((user, index) => (
                    <li key={index} className={cn("py-3 px-4 flex flex-row justify-between hover:bg-[#16181c] w-full transition-all items-center cursor-pointer",user.isGroup&&selectedUsers.length>0&&"bg-[#16181c] opacity-70")}
                        onClick={!user.isGroup  ? () => onUserClick(user.users[0]) :  selectedUsers.length==0? () => navigate('/Messages/'+user.id):()=>{}}>
                        <div className=" flex flex-row">
                            <Avatar className="mr-4">
                            <AvatarImage className="w-10 h-10 rounded-full border-[#ffffee46] border-[1px] border-solid" src={user.photo || "https://i.pinimg.com/736x/62/1d/bd/621dbd7d208d5c17498e0f73bf02aee8.jpg"} />
                            </Avatar><div className="flex flex-col h-full gap-0 ">
                                <h3 className="text-primary text-[15px]">{user.name}</h3>
                                <span className="text-gray">{!user.isGroup ? `@${user.users[0].userName}` : `${user.users.length} People`}</span>
                            </div>
                        </div>
                        {!user.isGroup&&selectedUsers && selectedUsers.filter(obj => obj['userName'] == user.users[0].userName).length > 0 && <div>
                            <Check className="w-4 h-4 text-secondary" />
                        </div>}
                    </li>
                ))
