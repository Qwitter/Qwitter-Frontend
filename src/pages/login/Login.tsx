...
export function Login() {
  ...
  const { mutateAsync: loginServicefn, isPending: loginServicePending } =
    useMutation({
  ...
  async function incrementStep(): Promise<void> {
    if (step == 1) {
      const { available }: { available: boolean | null } =
        await checkEmailExistence(form.getValues("email"));
      if (!available) {
        toast({
  ...
  const onSubmit = async (data: SignIn) => {
    const { avalible, message }: { avalible: boolean; message: string } =
      await loginServicefn(data);
    if (avalible) {
      form.reset();
    } else {
      toast({
    ...
    <>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        ...
        >
          ...
          {step === 1 && !checkEmailExistencePending && (
            <LoginEmail incrementStep={incrementStep} form={form} />
          )}
          {step === 2 && !loginServicePending && (
            <LoginPassword
