      ...
      {popUp.visible &&
        (popUp.content[0] === "@" || popUp.content[0] === "#") && (
          <div
            style={{
              ...{
                [mode == "Messages" ? "bottom" : "top"]:
                  mode === "Messages"
                    ? `${popUp.position.top + 5}px`
                    : `${popUp.position.top + 40}px`,
              },
              left: `${
          ...
          >
            <Avatar className="mr-4">
              <AvatarImage
                ...
                src={user.profileImageUrl || VITE_DEFAULT_IMAGE}
              />
            </Avatar>
...
function ShowTagsSuggestions({
  ...
  const { isPending, data, refetch } = useQuery<
    { count: number; text: string; entityId: string }[]
  >({
